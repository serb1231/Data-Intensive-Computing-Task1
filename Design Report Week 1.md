...
For the taxi trips, we required `passenger_count` and `trip_distance` to be positive. Fare columns are not checked, so 104,902 rows with a negative `fare_amount` remain in `trip_data`.
...
The integration pipeline (`build_integrated_trips`) enriches every cleaned trip with weather, air quality and pickup/dropoff zone information, writing `output_data/integrated_taxi_trips` partitioned by `year` and `month`. Trips are the anchor entity: every join is a `left` join from `trip_data`, so enrichment can only add columns, never remove or duplicate a trip — verified by comparing `trip_clean.count()` with the integrated row count.

**How should an hourly weather observation be associated with a taxi trip?**
Each trip's `tpep_pickup_datetime` is truncated down to the hour and joined on equality to the weather table's standardized `timestamp` (see Task 4). Weather is recorded hourly, so this is a clean 1-to-1 lookup: the trip inherits temperature, humidity, precipitation, snow depth, wind speed, pressure and condition code. Truncating _down_ deliberately picks the observation at or before pickup (a 14:45 pickup uses the 14:00 reading) rather than the nearest one — it is deterministic and avoids look-ahead. The source is a single station (8,784 rows = 366 days × 24 hours), so it is a citywide proxy by construction.

**How should hourly air-quality measurements be associated with a taxi trip?**
This requires a spatial decision before the temporal one. The raw EPA file is a _nationwide_ extract — 8,139,551 rows from 925 monitoring sites across 53 distinct `state_code` values, of which New York City is only 0.64%. Associating trips by hour alone would blend Californian readings into a New York taxi trip, and would silently mix time zones, since `timestamp_local` is Eastern in New York, Pacific in California and Atlantic in Puerto Rico. `air_quality_scope` therefore filters to the New York City boroughs (`state_code = 36`, `county_code` in 005/047/081) before any cleaning, after which `timestamp_local` is unambiguously Eastern and directly comparable to the trip timestamps. Scoping is deliberately a separate step from `air_quality_process`: the 8,087,666 discarded rows are valid measurements about other regions, so `execute_pipeline` reports them as `out_of_scope_records` rather than data-quality `rejected_records`.

The temporal association mirrors the weather strategy: the filtered data is truncated to `aq_hour`, grouped, and pivoted on `parameter_name` with `avg(sample_measurement)` — one column per pollutant, each keeping its own unit rather than being averaged across parameters. This yields exactly one row per hour, so the left join onto `pickup_hour` cannot fan out. PM2.5 (`parameter_code = 88101`) is the only pollutant in this extract, so `pm25` is currently the sole air-quality column.

We deliberately use a **citywide NYC hourly average** rather than matching each trip to its own borough's station: only Bronx, Brooklyn and Queens have any PM2.5 station (2 sites each), and those are precisely the boroughs that barely generate trips. Of the 9,554,778 raw Q1 pickups, Manhattan alone accounts for **89.55%** and has no station, while the three boroughs that do have stations supply only **10.05%** between them:

| Pickup borough |     Trips |  Share | PM2.5 station? |
| -------------- | --------: | -----: | -------------- |
| Manhattan      | 8,556,766 | 89.55% | no             |
| Queens         |   837,886 |  8.77% | yes            |
| Brooklyn       |    97,219 |  1.02% | yes            |
| Bronx          |    24,959 |  0.26% | yes            |
| Staten Island  |       234 |  0.00% | no             |

The remaining 0.40% (37,714 pickups) are EWR, zones marked unknown, and location IDs absent from the lookup. Borough-level matching would be false precision for roughly nine trips in ten while adding join complexity, so a citywide average is the honest representation of what these six stations can measure.

**How should missing observations be handled?**
All four enrichment joins are `left` joins, so an absent weather record, air-quality record or zone leaves the corresponding columns `null` rather than dropping the trip — consistent with the Task 4 rule that only rows missing their _own_ primary key or timestamp are eliminated. Values are deliberately **not** imputed: a null truthfully records "no observation", whereas an imputed one would be indistinguishable from a real measurement downstream. In practice the gaps are negligible. The NYC stations cover all 2,184 hours of January–March 2024, and exactly **18 of 8,480,540 rows (0.0002%)** have a null `pm25` — the same 18 that have a null `temp`. All 18 are trips whose own `tpep_pickup_datetime` is corrupt (2002, 2008, 2009 and 2023, visible as stray partitions in `trip_data`) and therefore lie outside every coverage window; `pickup_borough` and `dropoff_borough` have no nulls at all.

**What are the limitations of your integration strategy?**

- **Hourly granularity**: trips starting in the same hour all receive identical conditions, so those near an hour boundary lose precision.
- **Sparse sensor network, citywide averaging**: six stations in three boroughs averaged into one hourly figure, while Manhattan — 89.55% of pickups — has none, so for ~90% of trips the value is measured in a different borough. Weather is citywide for a different reason: it comes from one station.
- **Coarse spatial resolution**: enrichment is keyed on pickup hour and, for zones, on `LocationID`; the weather and air-quality values are never matched to where the trip actually was, so a rain cell over one borough or pollution on a single corridor is invisible.
- **Pickup-time only**: conditions at drop-off, which may differ meaningfully on longer trips, are not captured.
- **Single pollutant in practice**: the pivot is generic across pollutants, but this extract contains only PM2.5, and no composite index (e.g. an AQI) is computed.
- **Timezone handling**: `timestamp_local` is comparable to the trip timestamps only because every retained station is Eastern — the filter, not an explicit timezone conversion, is what makes this hold. Extending the pipeline to another city would require revisiting this rather than simply widening the filter.
