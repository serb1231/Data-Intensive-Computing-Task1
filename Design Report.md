...
The `taxi_zones` and `air_quality` datasets are treated as lookup tables. `taxi_zones` is a small, static dimension table (~265 rows). `air_quality` becomes a lookup table after ingestion filtering: the raw EPA extract is nationwide, but the taxi trips are NYC-only, so `air_quality_process` filters it to the New York City boroughs (`state_code = 36`, `county_code` in 005/047/081). What remains is ~52K rows from 6 stations carrying a single pollutant (`parameter_code = 88101`, PM2.5) — small and low-cardinality enough to behave as a lookup table. `weather` is not a lookup table; it is a time-series fact table, which is why it is partitioned by date below.
...
This requires a spatial decision before the temporal one. The raw EPA file is a _nationwide_ extract — 8.1M rows from ~289 monitoring sites across 53 states, of which New York City is only 0.64%. Associating trips by hour alone would blend Californian and Texan readings into a New York taxi trip, and would also silently mix time zones, since `timestamp_local` means Eastern in New York but Pacific in California. `air_quality_process` therefore first filters the data to the New York City boroughs (`state_code = 36`, `county_code` in 005/047/081). After this filter `timestamp_local` is unambiguously Eastern and directly comparable to the trip timestamps.
...
We deliberately use a **citywide NYC hourly average** rather than matching each trip to its own borough's station. The sensor network does not support finer resolution: only 3 of the 5 boroughs have any PM2.5 station at all (Bronx, Brooklyn, and Queens have 2 sites each; Manhattan and Staten Island have none), and Manhattan — which has no station — is the single largest source of yellow-taxi pickups. Borough-level matching would therefore be false precision for the majority of trips while adding join complexity, so a citywide average is the honest representation of what these six stations can actually measure.
...
In practice the gaps are negligible. The NYC stations cover all 2,184 hours of January–March 2024, so essentially every trip in the study period receives an air-quality value; the few nulls that remain come from trips whose own `tpep_pickup_datetime` is corrupt (the trip data contains a handful of records dated 2002, 2008, 2009 and 2023, visible as stray partitions in `trip_data`), which fall outside the weather and air-quality coverage windows entirely.
...
- **Sparse sensor network, citywide averaging**: after filtering, air quality comes from just six stations in three boroughs (Bronx, Brooklyn, Queens), averaged into a single hourly citywide figure. Manhattan and Staten Island have no PM2.5 station at all, yet Manhattan generates the largest share of pickups — so for most trips the value is a proxy from other boroughs rather than a local measurement. Weather has the same shape of limitation for a different reason: it comes from one station, so it is citywide by construction.
...
- **Timezone handling**: correctness relies on the NYC filter in `air_quality_process`. `timestamp_local` is only comparable to the trip timestamps because every retained station is in the Eastern timezone; the filter, not an explicit timezone conversion, is what makes this hold. Extending the pipeline to another city would require revisiting this rather than simply widening the filter.
